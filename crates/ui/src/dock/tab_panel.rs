...
/// Names a tab's close (X) button in the debug-bounds map, so a test can ask a
/// really-drawn frame whether a closable panel offered the control.
                ...
                visible
                    ...
                    .map(|ix| {
                        ...
                        let tab = Tab::new()
                            ...
                            // A per-tab close (X) button, shown only for a
                            // panel that reports itself closable. Stops
                            // propagation so the click never also selects the
                            // tab, and closes by id so it targets this tab
                        ...
                        // Hand the fully wired tab to the panel for the final
                        // say (restyle, swap the label, add/drop a suffix);
                        // a base panel with no skin handle takes the default.
                        match handle {
...
mod tests {
    ...
    /// A panel that reports a chosen `closable` and records whether the skin
    /// routed its tab through [`Panel::render_tab`].
    struct TabProbe {
        focus_handle: FocusHandle,
        closable: bool,
    ...
    /// Draw a two-tab group through the real [`DockSkin`] -- so the tab bar and
    /// its per-tab close buttons actually render -- and report whether the
    /// first tab offered a close button. The second tab is a non-closable
    /// filler, present only so the group has the two tabs a tab bar needs (a
    /// lone `Auto` panel draws a plain title); it never draws a button of its
    /// own, so the debug-bounds probe is unambiguous.
    ...
    /// The gate is real: a panel that reports itself non-closable draws none.
    /// Without this the test above would also pass a skin that always drew one.
    #[gpui::test]
    ...
    /// The skin hands each tab to its panel's [`Panel::render_tab`], so a panel
    /// gets the final say over its own tab.
    #[gpui::test]
